...
router.get("/:id", (req, res) => {
  ...
  Users.findById(id)
    .then((user) => {
      user ? res.json(user) : res.status(404).json({ message: "Could not find a user with the given id" });
    })
    .catch((err) => {
...
router.get("/:id/decks", (req, res) => {
  ...
  Users.findDeckByUserId(id)
    .then((data) => {
      data ? res.status(200).json(data) : res.status(404).json({ errMessage: "Cannot find user data." });
    })
    .catch((err) => {
...
router.delete("/:id", (req, res) => {
  ...
  Users.remove(id)
    .then((deleted) => {
      deleted ? res.json({ removed: deleted }) : res.status(404).json({ message: "Could not find a user with the given id" });
    })
    .catch((err) => {
